...
dict_of_disease_states = {'severe_heart_failure': 1823, 'moderate_heart_failure': 1822, 'mild_heart_failure': 1821,
                          'asymptomatic_angina': 3102, 'mild_angina': 1818, 'moderate_angina': 1819,
                          'severe_angina': 1820,
...
def prevalence_plot(me_id, disease, sex_id):
    ...
    out_path = '/share/costeffectiveness/CEAM/gbd_ms_function_output_plots/{d}_prevalence_among_sex_id{s}.pdf'.format(
        d=disease, s=sex_id)
    ...
    for i in range(0, number_of_distinct_pops):
        ...
        prev_dict['{s}_{i}'.format(s=sex_id, i=i)]['prevalence_{}'.format(i)] = \
        prev_dict['{s}_{i}'.format(s=sex_id, i=i)]['count_sick'] / prev_dict['{s}_{i}'.format(s=sex_id, i=i)][
            'count_total']

    merged = pd.merge(prev_dict['{}_0'.format(sex_id)], prev_dict['{}_1'.format(sex_id)], on=['age', 'sex_id'],
                      how='outer')
    ...
    plt.errorbar(merged.age.values, merged.mean_prev.values, yerr=[merged.mean_prev.values - merged.lower.values,
                                                                   merged.upper.values - merged.mean_prev.values],
                 fmt='o',
