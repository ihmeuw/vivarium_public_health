...
class RotaVaccine():
    ...
    def _determine_who_gets_first_dose(self, event):
        ...
        self.population_view.update(children_who_will_receive_third_dose[[self.vaccine_working_column]])
    ...
    def incidence_rates(self, index, rates, population_view):
        ...
        if self.active == True:
            ...
            if population.query("{} == 1".format(self.vaccine_working_column)).empty:
                ...
                rates_df = pd.DataFrame({'rates': rates})

                pop = population.query("{} == 1".format(self.vaccine_working_column))
 
                rates_df = rates_df.join(pop)

                rates_df['new_rate'] = rates_df['rates'] * vaccine_effectiveness

                return pd.Series(rates_df['new_rate'])
