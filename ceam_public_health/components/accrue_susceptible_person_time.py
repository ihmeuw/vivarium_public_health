...
class AccrueSusceptiblePersonTime():
    ...
    def setup(self, builder):
        ...
        age_bins = ezfuncs.query('''select age_group_id, age_group_years_start, age_group_years_end from age_group''', conn_def='shared')
    ...
    @uses_columns(['diarrhea', 'susceptible_person_time_under_early_neonatal', 'susceptible_person_time_late_neonatal', 'susceptible_person_time_post_neonatal', 'age'], 'alive')
    def count_time_steps_sim_has_diarrhea(event):
        ...
        for key, value in self.dict_of_age_group_name_and_max_values:
            # FIXME: Need to figure out how to accrue person time for chronic diseases
