...
# use generate test population everywhere that you can
from ceam_tests.util import generate_test_population

def test_DiarrheaEtiologyState(get_disease_states_mock):
    ...
    # give men 100% incidence and then we'll ensure that only men get disability for diarrhea
    incidence_table = incidence_table.loc[incidence_table.sex == 'Male', 'rate'] = 1

    transition = RateTransition(sick, 'diarrhea_due_to_rotaviral_entiritis', incidence_table)
    ...
    transition.base_incidence = simulation.tables.build_table(build_table(0.0))

    incidence_rate = simulation.values.get_rate('incidence_rate.test_incidence')

    pump_simulation(simulation, iterations=1)

    # check the population table to make sure that men are in the diarrhea state with disability weight


# def test_ApplyDiarrheaExcessMortality():
    # make sure that every person with diarrhea has an excess mortality rate that is counted in the mortality rate dataframe
...
# patch tells you to return a different output of whatever function you are calling from patch. replaces output with a dummy that you can manipulate.
