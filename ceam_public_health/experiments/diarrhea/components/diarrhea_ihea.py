...
def build_diarrhea_model():
    ...
    no_care_sought = DiseaseState('no_care_sought',
    ...
    diarrhea.add_transition(no_care_sought, proportion=1-CARE_PROPORTION)
    diarrhea.add_transition(care_sought, proportion=CARE_PROPORTION)

    # Each sequela state will transition back to healthy once the dwell time
    # associated with the state has passed.
    no_care_sought.add_transition(healthy)
    ...
    no_care_sought.cleanup_function = freeze_etiologies
    care_sought.cleanup_function = freeze_etiologies

    return [etiology.model for etiology in etiologies] + [
        DiseaseModel('diarrhea',
                     states=[healthy, diarrhea, care_sought, no_care_sought],
