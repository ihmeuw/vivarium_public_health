...
def factory():
    ...
    # TODO: need to model severity splits for stroke. then we can bring in correct disability weights
    # (dis weights correspond to healthstate ids which correspond to sequela)
    hemorrhagic_stroke = make_disease_state(causes.hemorrhagic_stroke,
                                            dwell_time=28,
                                            side_effect_function=hospitalization_side_effect_factory(
                                                0.52, 0.6, 'hemorrhagic stroke'))  # rates as per Marcia e-mail
    ischemic_stroke = make_disease_state(causes.ischemic_stroke,
                                         dwell_time=28,
                                         side_effect_function=hospitalization_side_effect_factory(
                                             0.52, 0.6, 'ischemic stroke'))  # rates as per Marcia e-mail
