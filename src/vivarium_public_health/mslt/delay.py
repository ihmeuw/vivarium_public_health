...
class DelayedRisk:
    ...
    def __init__(self, name: str):
        ...
        self.configuration_defaults = {
            name: {"constant_prevalence": False, "tobacco_tax": False, "delay": 20,},
        }
    ...
    def setup(self, builder):
        ...
        mort_rr_data = pivot_load(
            builder, f"risk_factor.{self.name}.mortality_relative_risk"
        )
        ...
        dis_rr_data = pivot_load(
            builder, f"risk_factor.{self.name}.disease_relative_risk"
        )
        ...
        for disease in diseases:
            ...
            bau_col = {
                c: c.replace(dis_prefix, bau_prefix).replace("post_", "")
                for c in dis_columns
            }
            int_col = {
                c: c.replace(dis_prefix, int_prefix).replace("post_", "")
                for c in dis_columns
            }
