...
class DiseaseObserver:
    ...
    configuration_defaults = {
        "metrics": {
            "disease_observer": {
                ...
                "by_year": False,
                "by_sex": False,
                "sample_prevalence": {"sample": False, "date": {"month": 7, "day": 1,}},
            }
        }
    }
    ...
    def __init__(self, disease: str):
        ...
        self.configuration_defaults = {
            "metrics": {
                f"{disease}_observer": DiseaseObserver.configuration_defaults[
                    "metrics"
                ]["disease_observer"]
            }
        }
    ...
    def on_time_step_prepare(self, event):
        ...
        if self._should_sample(event.time):
            point_prevalence = get_prevalent_cases(
                pop, self.config.to_dict(), self.disease, event.time, self.age_bins
            )
