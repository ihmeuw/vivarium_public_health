...
class OutputTemplate(Template):
    ...
    def substitute(*args, **kws) -> Union[SubstituteString, "OutputTemplate"]:
        ...
        mapping = {
            key: self.format_template_value(value) for key, value in mapping.items()
        }
        try:
...
def get_age_bins(builder) -> pd.DataFrame:
    ...
    min_bin_start = age_bins.age_start[
        np.asscalar(np.digitize(age_start, age_bins.age_end))
    ]
...
def get_output_template(
    by_age: bool, by_sex: bool, by_year: bool, **_
) -> OutputTemplate:
...
def get_group_counts(
    ...
    for group, age_group in ages:
        ...
        for sex in sexes:
            ...
            in_group = (
                pop.query(group_filter) if group_filter and not pop.empty else pop
            )
...
def get_susceptible_person_time(
    pop, config, disease, current_year, step_size, age_bins
):
    base_key = get_output_template(**config).substitute(
        measure=f"{disease}_susceptible_person_time", year=current_year
    )
    base_filter = QueryString(
        f'alive == "alive" and {disease} == "susceptible_to_{disease}"'
    )
...
def get_lived_in_span(
    ...
    """
    lived_in_span = pop.loc[
        (t_start < pop["exit_time"]) & (pop["entrance_time"] < t_end)
    ]
...
def get_person_time_in_span(
    ...
    for group, age_bin in ages:
        ...
        for sex in sexes:
            ...
            in_group = (
                lived_in_span.query(group_filter)
                if group_filter
                else lived_in_span.copy()
            )
...
def get_deaths(
    ...
    """
    base_filter = QueryString(
        'alive == "dead" and cause_of_death == "death_due_to_{cause}"'
    )
    ...
    for year, (t_start, t_end) in time_spans:
        ...
        for cause in causes:
            cause_year_key = base_key.substitute(
                measure=f"death_due_to_{cause}", year=year
            )
...
def get_years_of_life_lost(
    ...
    for year, (t_start, t_end) in time_spans:
        ...
        for cause in causes:
            cause_year_key = base_key.substitute(
                measure=f"ylls_due_to_{cause}", year=year
            )
            cause_filter = base_filter.format(cause=cause)
