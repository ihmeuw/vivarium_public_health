...
class Mortality:
    ...
    def setup(self, builder):
        all_cause_mortality_data = builder.data.load(
            "cause.all_causes.cause_specific_mortality_rate"
        )
        self.all_cause_mortality_rate = builder.lookup.build_table(
            all_cause_mortality_data,
            key_columns=["sex"],
        ...
        self.life_expectancy = builder.lookup.build_table(
            life_expectancy_data, parameter_columns=["age"]
        )
    ...
    def on_initialize_simulants(self, pop_data):
        pop_update = pd.DataFrame(
            {"cause_of_death": "not_dead", "years_of_life_lost": 0.0},
