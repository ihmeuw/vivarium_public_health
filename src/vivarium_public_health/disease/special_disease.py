...
class RiskAttributableDisease:
    ...
    def __init__(self, cause, risk):
        ...
        self.susceptible_event_time_column = (
            f"susceptible_to_{self.cause.name}_event_time"
        )
        ...
        self._transition_names = [
            f"susceptible_to_{self.cause.name}_TO_{self.cause.name}"
        ]
        ...
        new_pop = pd.DataFrame(
            {
                ...
                self.diseased_event_time_column: pd.Series(
                    pd.NaT, index=pop_data.index
                ),
                self.susceptible_event_time_column: pd.Series(
                    pd.NaT, index=pop_data.index
                ),
            },
    ...
    def compute_disability_weight(self, index):
        ...
        disability_weight.loc[with_condition] = self.base_disability_weight(
            with_condition
        )
    ...
    def get_exposure_filter(self, distribution, exposure_pipeline, threshold):
        ...
        if distribution in [
            ...
            if len(threshold_op) != 1 or not allowed_operator.intersection(
                threshold_op
            ):
                raise ValueError(
                    f"Your {threshold} is an incorrect threshold format. It should include "
    ...
    def load_excess_mortality_rate_data(self, builder):
        if builder.configuration[self.cause.name].mortality:
            emr_data = builder.data.load(
                f"cause.{self.cause.name}.excess_mortality_rate"
            )
        else:
            emr_data = 0
        return emr_data
