...
class DiseaseModel(Machine):
    ...
    def setup(self, builder):
        ...
        self.randomness = builder.randomness.get_stream(
            f"{self.state_column}_initial_states"
        )
    ...
    def load_cause_specific_mortality_rate_data(self, builder):
        if "cause_specific_mortality_rate" not in self._get_data_functions:
            only_morbid = builder.data.load(f"cause.{self.cause}.restrictions")[
                "yld_only"
            ]
            if only_morbid:
                csmr_data = 0
    ...
    def assign_initial_status_to_simulants(
        ...
        initial_states = pd.Series(
            np.array(state_names)[choice_index], index=simulants.index
        )
