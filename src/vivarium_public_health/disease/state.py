...
class BaseDiseaseState(State):
    ...
    def on_initialize_simulants(self, pop_data):
        ...
        pop_update = pd.DataFrame(
            {self.event_time_column: pd.NaT, self.event_count_column: 0},
    ...
    def add_transition(
        ...
        **kwargs,
        ...
        if not source_data_type:
            ...
            t = transition_map[source_data_type](
...
class DiseaseState(BaseDiseaseState):
    ...
    def load_excess_mortality_rate_data(self, builder):
        ...
        if "excess_mortality_rate" in self._get_data_functions:
            return self._get_data_functions["excess_mortality_rate"](
                self.cause, builder
            )
        elif only_morbid:
            return 0
            ...
            return builder.data.load(
                f"{self.cause_type}.{self.cause}.excess_mortality_rate"
            )
