...
def load_relative_risk_data(
    ...
    relative_risk_source = builder.configuration[
...
def get_population_attributable_fraction_data(
    ...
    if (
        ...
        relative_risk_data = get_relative_risk_data(builder, risk, target).set_index(
            key_cols
        )
...
def validate_distribution_data_source(builder, risk: EntityString):
    ...
    if risk.type == "alternative_risk_factor":
        ...
        if not category_thresholds:
            raise ValueError(
                "Must specify category thresholds to use alternative risks."
            )
...
def validate_relative_risk_data_source(
    ...
    provided_keys = set(
        k
        for k, v in relative_risk_source.to_dict().items()
...
def validate_rebin_source(builder, risk: EntityString, data: pd.DataFrame):
    ...
    if (
        ...
        and builder.configuration[risk.name]["category_thresholds"]
    ):
        raise ValueError(
