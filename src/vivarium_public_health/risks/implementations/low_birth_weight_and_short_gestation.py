...
class LBWSGDistribution:
    ...
    def _get_categorical_index(self, exposure):
        exposure_bw_gt_index = exposure.set_index(
    ...
    def _convert_to_continuous(self, categorical_exposure):
        ...
        values = categorical_exposure.reset_index().apply(
            single_values_from_category, axis=1
        )
        return pd.DataFrame(
            list(values),
...
def get_categories_by_interval(builder, risk):
    ...
    idx = pd.MultiIndex.from_tuples(
        cats.name.apply(get_intervals_from_name),
...
class LBWSGRiskEffect:
    ...
    configuration_defaults = {
        "effect_of_risk_on_target": {"measure": {"relative_risk": None,}}
    }
    ...
    def setup(self, builder):
        ...
        self.exposure_effect = data_transformations.get_exposure_effect(
            builder, self.risk
        )
