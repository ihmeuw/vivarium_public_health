...
class DummyRiskEffect(RiskEffect):
    ...
    def _get_paf_data(self, builder):
        if 'paf' in self._get_data_functions:
            ...
            if isinstance(exposure, str):
                exposure = builder.data.load(f'covariates.{exposure}.estimate')
            rr = builder.configuration[self.risk][self.affected_entity_type][self.affected_entity]
    ...
    def _get_rr_data(self, builder):
        if 'rr' in self._get_data_functions:
            ...
            rr_data = pivot_age_sex_year_binned(rr_data, 'parameter', 'value')
        else:
            rr_data = builder.configuration[self.risk][self.affected_entity_type][self.affected_entity]
        return rr_data
