...
from ceam_public_health.components.interventions.rota_vaccine import set_vaccine_duration, determine_vaccine_protection, wane_immunity 
...
def test_determine_vaccine_protection():
    ...
    series = determine_vaccine_protection(population, dose_working_index, wane_immunity, simulation.current_time, "first", effectiveness)
...
def test_determine_vaccine_protection2():
    ...
    population = simulation.population.population
    pop = population.copy()
    pop['days_since_vaccination'] = simulation.current_time - pop['rotaviral_entiritis_vaccine_third_dose_immunity_start_time']
    ...
    series = determine_vaccine_protection(population, dose_working_index, wane_immunity, simulation.current_time, "third", effectiveness)
...
def test_wane_immunity():
    assert np.allclose(.25, wane_immunity(30, 20, 20, .5)), "vaccine should confer 50% as much benefit when halfway through the waning period"
    assert np.allclose(0, wane_immunity(41, 20, 20, .5)), "vaccine should confer no benefit when after the waning period is over"
...
def test_rota_vaccine_coverage():
    ...
    assert len(simulation.population.population.rotaviral_entiritis_vaccine_first_dose_is_working) > 0, "ensure that, even when there is no intervention, there is still some baseline coverage that exists"

    # now create a simulation where we are including an intevention
    simulation2 = setup_simulation([generate_test_population, age_simulants, RotaVaccine(True)], start=datetime(2014, 1, 1), population_size=10000)
