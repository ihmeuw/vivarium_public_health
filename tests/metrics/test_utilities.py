...
def ages_and_bins(request):
    ...
    bin_ages, step = np.linspace(
        age_min, age_max, age_groups, endpoint=False, retstep=True
    )
...
def test_get_age_sex_filter_and_iterables(ages_and_bins, observer_config):
    ...
    if observer_config["by_age"] and observer_config["by_sex"]:
        assert (
            age_sex_filter
            == '{age_start} <= age and age < {age_end} and sex == "{sex}"'
        )

        for (g1, s1), (g2, s2) in zip(
...
def test_get_susceptible_person_time(ages_and_bins, sexes, observer_config):
    ...
    pop = pd.DataFrame(
        list(product(ages, sexes, states)), columns=["age", "sex", disease]
    )
    pop["alive"] = "alive"
    # Shuffle the rows
...
def test_get_disease_event_counts(ages_and_bins, sexes, observer_config):
    ...
    pop = pd.DataFrame(
        list(product(ages, sexes, states)),
        columns=["age", "sex", f"{disease}_event_time"],
    )
    # Shuffle the rows
    pop = pop.sample(frac=1).reset_index(drop=True)

    counts = get_disease_event_counts(
        pop, observer_config, disease, event_time, age_bins
    )
...
def test_get_lived_in_span():
    ...
    pop = pd.DataFrame(
        {"age": to_years(10 * dt), "entrance_time": starts, "exit_time": ends}
    )
...
def test_get_lived_in_span_no_one_in_span():
    ...
    pop = pd.DataFrame(
        {
            "entrance_time": t_start - 2 * dt,
            "exit_time": t_start - dt,
            "age": range(100),
        }
    )
    lived_in_span = get_lived_in_span(pop, t_start, t_end)
...
def test_get_person_time_in_span(ages_and_bins, observer_config):
    ...
    if observer_config["by_age"]:
        for group, age_bin in age_bins.iterrows():
            group_pt = sum(
                set([v for k, v in pt.items() if f"in_age_group_{group}" in k])
            )
            if observer_config["by_sex"]:
...
def test_get_age_bins(
    ...
    age_start,
    exit_age,
    result_age_end_values,
    result_age_start_values,
):
    base_config.update(
        {"population": {"age_start": age_start, "exit_age": exit_age}},
        **metadata(__file__),
    )
