...
# TEST 7 -- test that diarrhea csmr is deleted from the background mortality rate
def test_cause_deletion():
    ...
    assert np.allclose(np.sort(simulation_values), np.sort(cause_deleted_mr_values * ts/365), atol=.000001), "make sure diarrhea has been deleted from the background mortality rate"
