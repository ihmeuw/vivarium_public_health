...
def get_test_prevalence(simulation, key):
    ...
    """
    try:
        simulants_status_counts = (
            simulation.get_population().test.value_counts().to_dict()
        )
        result = float(
            simulants_status_counts[key] / simulation.get_population().test.size
        )
    except KeyError:
        result = 0
    return result
...
def test_dwell_time(assign_cause_mock, base_config, base_plugins, disease, base_data):
    ...
    assert np.all(
        simulation.get_population().event_event_time == pd.to_datetime(event_time)
    )
...
def test_prevalence_single_state_with_migration(
    ...
    error_message = (
        "initial status of simulants should be matched to the prevalence data."
    )
    assert np.isclose(
...
def test_prevalence_multiple_sequelae(
    ...
    base_config.update(
        {"population": {"population_size": 100000}}, **metadata(__file__)
    )
    simulation = InteractiveContext(
        components=[TestPopulation(), model],
        configuration=base_config,
...
def test_prevalence_single_simulant():
    ...
    simulants_df = pd.DataFrame(
        {"sex": "Female", "age": 3, "sex_id": 2.0}, index=test_index
    )
...
def test_mortality_rate(base_config, base_plugins, disease):
    ...
    model = DiseaseModel(
        disease, initial_state=healthy, states=[healthy, mortality_state]
    )
...
def test_incidence(base_config, base_plugins, disease):
    ...
    transition = RateTransition(
        ...
        output_state=sick,
        get_data_functions={
            "incidence_rate": lambda _, builder: builder.data.load(key)
        },
    )
