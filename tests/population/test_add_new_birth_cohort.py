...
def config(base_config):
    ...
    base_config.update(
        {
            "population": {"population_size": 10000, "age_start": 0, "age_end": 125,},
            "time": {"step_size": 10,},
        },
...
def test_FertilityCrudeBirthRate(config, base_plugins):
    ...
    simulation._data.write(
        "covariate.live_births_by_sex.estimate", crude_birth_rate_data()
    )
    ...
    config.update(
        {
            ...
            "time": {"start": {"year": 2016}, "end": {"year": 2025},},
        }
    )
    components = [TestPopulation(), FertilityCrudeBirthRate()]
...
def test_FertilityCrudeBirthRate_extrapolate(base_config, base_plugins):
    base_config.update(
        {
            ...
            "time": {"start": {"year": 2016}, "end": {"year": 2026}, "step_size": 365,},
        }
    )
    ...
    simulation._data.write(
        ...
        crude_birth_rate_data(live_births_by_sex),
    )
    simulation.setup()
