...
from ceam_inputs import (get_ors_exposures, get_ors_pafs,
...
def test_determine_who_gets_ors():
    ...
    num_males_with_diarrhea = len(pop.query(
        "sex == 'Male' and diarrhea != 'healthy'"))

    ors_proportion = len(pop.query(
        "ors_working == 1 and sex == 'Male'"))/num_males_with_diarrhea

    no_ors_proportion = (len(pop.query(
        "ors_working == 0 and sex == 'Male' and diarrhea != 'healthy'")) /
        num_males_with_diarrhea)

    ors_exposure = get_ors_exposures()

    GBD_proportion_exposed = ors_exposure.query("sex == 'Male' and age <.01")\
        .set_index(['year']).get_value(2010, 'cat2')
    GBD_proportion_not_exposed = ors_exposure.query("sex == 'Male' and age <.01")\
        .set_index(['year']).get_value(2010, 'cat1')
...
def test_mortality_rates():
    ...
    excess_mortality_rate = simulation.values.get_rate(
    ...
    diarrhea_but_no_ors_pop = pop.query(
        "ors_working == 0 and diarrhea != 'healthy'")

    pafs = get_ors_pafs()
    GBD_paf = pafs.query(
        "sex == 'Male' and age <.01").set_index(['year']).get_value(2010, 'paf')
    ...
    GBD_mr = mr.query(
        "sex == 'Male' and age <.01").set_index(['year']).get_value(2010, 'rate')

    no_ors_diarrhea_mortality_rate = GBD_mr * (1 - GBD_paf) * GBD_rr * 1/365
    ors_diarrhea_mortality_rate = GBD_mr * (1 - GBD_paf) * 1/365

    simulation_no_ors_rate = excess_mortality_rate(
        diarrhea_but_no_ors_pop.index).unique()
    simulation_ors_rate = excess_mortality_rate(ors_pop.index).unique()

    assert np.allclose(no_ors_diarrhea_mortality_rate, simulation_no_ors_rate,
                       rtol=.05)

    assert np.allclose(ors_diarrhea_mortality_rate, simulation_ors_rate,
                       rtol=.05)
