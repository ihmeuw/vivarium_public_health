...
def test_RiskEffect(base_config):
    ...
    simulation = setup_simulation([TestPopulation(), effect], input_config=base_config)
...
def test_continuous_exposure_effect(base_config):
...
def test_CategoricalRiskComponent_dichotomous_case(get_exposure_mock, get_paf_mock,
                                                   get_rr_mock, get_mf_mock, base_config):
    year_start = base_config.time.start.year
    year_end = base_config.time.end.year
    time_step = pd.Timedelta(days=base_config.time.step_size)
    ...
    simulation = setup_simulation([TestPopulation(), component], input_config=base_config)
    simulation.step()
...
def test_ContinuousRiskComponent(get_exposure_mock, get_rr_mock, get_paf_mock, get_mf_mock,
                                 get_distribution_mock, base_config):
    time_step = pd.Timedelta(days=base_config.time.step_size)
    year_start = base_config.time.start.year
    year_end = base_config.time.end.year
...
def test_correlated_propensity(load_risk_corr_mock, base_config):
    ...
    for risk in [
        ...
        propensities.append(correlated_propensity_factory(base_config)(pop, risk))
...
@pytest.mark.skip
@pytest.mark.slow
def test_correlated_exposures(load_rc_matrices_mock, base_config):
    ...
    for i in range(100):
        ...
        base_config.update({'population': {'population_size': 100000}}, layer='override')
        simulation = setup_simulation([TestPopulation()] + risks, base_config)
