...
def make_test_data_table(values: List, parameter="cat") -> pd.DataFrame:
    ...
    if len(values) == 1:
        df = build_table(
            values[0], year_start, year_end, ("age", "year", "sex", "value")
        )
    else:
        cats = (
...
def polytomous_risk():
    ...
    exposure_data = build_table(
        ...
        year_start,
        year_end,
        ["age", "year", "sex", "cat1", "cat2", "cat3", "cat4"],
    ).melt(
        id_vars=("age_start", "age_end", "year_start", "year_end", "sex"),
        var_name="parameter",
        value_name="value",
...
def coverage_gap():
    ...
    paf = (rr * cg_exposed + (1 - cg_exposed) - 1) / (
        rr * cg_exposed + (1 - cg_exposed)
    )
    ...
    paf_data = build_table(
        ...
        year_start,
        year_end,
        ["age", "year", "sex", "population_attributable_fraction"],
    ).melt(
        id_vars=("age_start", "age_end", "year_start", "year_end", "sex"),
